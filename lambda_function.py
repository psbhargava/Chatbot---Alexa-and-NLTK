...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech = ('Welcome to buybay. We help you buy Pets Laptops?')
        reprompt = "Are you looking for a Pet or Dog?"
...
class InProgressBuyLaptopIntent(AbstractRequestHandler):
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return (is_intent_name("BuyLaptopIntent")(handler_input)
                and handler_input.request_envelope.request.dialog_state != DialogState.COMPLETED)

    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        logger.info("In InProgressBuyLaptopIntent")
        current_intent = handler_input.request_envelope.request.intent
        prompt = ""
            ...
            if slot_name not in ["article", "at_the", "I_Want"]:
                if (current_slot.confirmation_status != SlotConfirmationStatus.CONFIRMED
                    ...
                    if current_slot.resolutions.resolutions_per_authority[0].status.code == StatusCode.ER_SUCCESS_MATCH:
                        ...
                        if current_slot.name in req_laptop_slots:
                            prompt = "What {} are you looking for?".format(current_slot.name)
...
class CompletedBuyLaptopIntent(AbstractRequestHandler):
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return (is_intent_name("BuyLaptopIntent")(handler_input)
            and handler_input.request_envelope.request.dialog_state == DialogState.COMPLETED)
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        logger.info("In CompletedBuyLaptopIntent")
        filled_slots = handler_input.request_envelope.request.intent.slots
        slot_values = get_slot_values(filled_slots)
        speech = ("I found a laptop for {} "
                          "{} "
                          "{} "
                          ". Laptop name "
                          "{}"
                          " intel G50".format(
                    slot_values["laptopram"]["resolved"],
                    slot_values["laptopsize"]["resolved"],
                    slot_values["laptopbrand"]["resolved"],
                    slot_values["laptopbrand"]["resolved"]))
...
req_laptop_slots = ["laptopram", "laptopsize","laptopbrand"]
...
sb.add_request_handler(InProgressBuyLaptopIntent())
sb.add_request_handler(CompletedBuyLaptopIntent())
